...
runLine(str){
	if (str.substring(0,1)=='.') localCommand(str.substring(1).split(' '));
	else compiler.runCommandLine(str);

	function localCommand(l){
		if(l[0]=='saveas') saveAs(l[1]);
	}
...
constructor(){
	...
	cc.selectionStart =  cc.value.length; 
	cc.selectionEnd = cc.value.length;
...
//	 	if(e.keyCode==70) {e.preventDefault(); e.stopPropagation(); procs.focus();}
	...
	function handlecr(e){
		...
		setTimeout(function(){commandCenter.runLine(str);},100);
	}
